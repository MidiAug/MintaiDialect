...
class UnifiedVoice(nn.Module):
    ...
    def get_conditioning(self, speech_conditioning_input, cond_mel_lengths=None, speaker_ids=None):
        ...
        if speaker_ids is not None:
            ...
            spk_conds = torch.cat(conds_list, dim=0)
            # print("####> spk_conds: {}".format(spk_conds.size()))
            # # ####> spk_conds: torch.Size([1, 32, 1280])
            return spk_conds
        
        # 如果设置了单一的 mean_condition，直接返回固定的条件向量
        if hasattr(self, 'mean_condition') and self.mean_condition is not None:
            logger.info("###>> Branch: mean_condition")
        ...
        if self.condition_type == "perceiver":
            ...
            if self.condition_type == "conformer_perceiver":
                ...
                # print("####> conformer_perceiver conds: {}".format(conds.size()))
                # # ####> conformer_perceiver conds: torch.Size([1, 32, 1280])
