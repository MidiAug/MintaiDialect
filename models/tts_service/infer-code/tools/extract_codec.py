...
def split_dataset(
    ...
    # 计算分割点
    valid_size = int(len(lines) * (1 - train_ratio))
        ...
        for line in tqdm(f, desc="处理音频文件"):
            ...
            try:
                ...
                if len(audio.shape) > 1 and audio.shape[0] > 1:
                    audio = audio.mean(dim=0, keepdim=True)  # shape [1, time]
            except Exception as e:
...
def main():
    ...
    # 分割数据集
    split_dataset(metadata_file, output_dir, train_ratio=TRAIN_SPLIT_RATIO)
