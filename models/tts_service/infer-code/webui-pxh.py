...
import argparse
parser = argparse.ArgumentParser(description="ZhTTS WebUI")
...
MODE = 'local'
speaker_info_path = os.path.join(cmd_args.model_dir, "speaker_info.json")
tts = IndexTTS(model_dir=cmd_args.model_dir, cfg_path=os.path.join(cmd_args.model_dir, "config.yaml"), speaker_info_path=speaker_info_path)
...
def gen_single(text, infer_mode,
               max_text_tokens_per_sentence=120, sentences_bucket_max_size=4,
                *args, progress=gr.Progress()):
    speaker_id = None
    prompt = "/data01/ttsdata/Pvocaldata/vocaldata/1920x1080/2247494027_1/clips/2247494027_1_6_vocals.wav"
    output_path = None
    if not output_path:
    ...
    if infer_mode == "普通推理":
        ...
        # 批次推理
        output = tts.infer_fast(prompt, text, output_path, verbose=cmd_args.verbose, speaker_id=speaker_id,
            max_text_tokens_per_sentence=int(max_text_tokens_per_sentence),
...
with gr.Blocks(title="pxh-speech-generate Demo") as demo:
    mutex = threading.Lock()
    gr.HTML('''
    <h2><center>莆仙话生成系统</h2>
<p align="center">
</p>
    ''')
    prompt_audio = "sample_prompt.wav"
    with gr.Tab("音频生成"):
        with gr.Row():
            os.makedirs("prompts", exist_ok=True)
        ...
        with gr.Accordion("高级生成参数设置", open=False):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        repetition_penalty = gr.Number(label="repetition_penalty", precision=None, value=10.0, minimum=0.1, maximum=50.0, step=0.1)
    ...
    gen_button.click(gen_single,
                     inputs=[input_text_single, infer_mode,
